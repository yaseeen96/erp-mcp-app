  ...
  return server.prompt(
    {
      ...
      description:
        "Search the web, talk through scope, then save real tasks with add-tasks. Does not check in.",
      schema: z.object({
      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              "4. Call add-tasks with projects: [{ name, tasks: [{ description, estimated_time }] }]. That does not check me in.",
              "5. Call check-in only if I explicitly ask to check in, punch in, or start the day.",
            ]
              .filter(Boolean)
              .join("\n"),
          },
